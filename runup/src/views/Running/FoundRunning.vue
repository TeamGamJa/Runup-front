<template>
    <div class="Running-container">
        <div class="Running-header">
            <v-row class="Running-category">
                ...
                <v-col cols="3">
                <v-select v-model="RunningMcategory" id="Running-Mcategory" label="중분류" :items="categorymMedium" @change="inputSelectVal"></v-select>
            </v-col>
            <v-col cols="3">
    ...
    <v-card-title>
      ...
      <v-text-field v-model="search" label="검색" single-line hide-details ></v-text-field>
        ...
        <v-data-table :headers="headers" :items="runningList" class="elevation=1">
            <template v-slot:items="props">
                <td>{{ props.item.RunningNum }}</td>
                <td class="text-xs-center"><router-link :to="{ name: 'DetailRunning', props: { id: props.item.RunningNum } }">{{ props.item.RunningTitle }}</router-link></td>
  ...
  methods: {
    inputSelectVal(value) {
     ...
     updateRunningList(list) {
        ...
        while (this.runningList.length <= 7) {
