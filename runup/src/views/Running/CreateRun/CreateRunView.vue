<template>
      ...
      <v-col>
        <v-sheet height="64">
          <v-toolbar flat>
            ...
            <v-btn outlined class="ml-4" color="grey darken-2" @click="addEvent">
              Add Event
            </v-btn>
          </v-toolbar>
        </v-sheet>
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-menu offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-text-field v-bind="attrs" v-on="on" label="Color" readonly></v-text-field>
          </template>
          <v-list>
            <v-list-item v-for="(color, index) in colors" :key="index" :value="color" @click="eventColor = color">
